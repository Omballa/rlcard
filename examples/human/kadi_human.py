        ...
        if player == 0:
            ...
            for i, a in enumerate(legal):
                if a == -1:
                    ...
                    card_str = state['hand'][a] if a < len(state['hand']) and a >= 0 else str(a)
                    print(f"  [{i}] Play card index {a} -> {card_str}")
            print(f"Legal actions for opponent (Player {player}): {legal}")
            raw_action = int(np.random.choice(legal))

            if raw_action == -1:
                print(f"Opponent (Player {player}) chose: DRAW")
            else:
                # try to map action index to card string (guard bounds)
                card_str = state_before['hand'][raw_action] if 0 <= raw_action < len(state_before['hand']) else str(raw_action)
                print(f"Opponent (Player {player}) played: {card_str}")
